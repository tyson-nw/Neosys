...
@can('create_page')
<div>
    <a href='/spells/create' class='rounded-full bg-blue-300 p-1 px-3 m-1'>Create new spell</a>
</div>
@endcan
...
<ul class='list-disc m-5 mx-8' >
    @foreach($spells as $spell)
        <li>
            <a href='/spell/{{$page->slug}}' class='underline text-xl'>{{$page->title}}</a> 
            @can('delete_page')
                <form method='POST' action='/spell/{{$page->slug}}' class='inline'> @csrf @method('delete') <button type='submit' class='rounded-full bg-blue-300 p-1  px-3 m-1'>Delete</button>
            @endcan
        </form>
    @endforeach
</ul>
